...
export const CalendarSyncView: React.FC<CalendarSyncViewProps> = ({ reservations, onSyncComplete }) => {
    ...
    const [targetEmail, setTargetEmail] = useState("salacalacutaweb@gmail.com");
    ...
    const handleSyncFrom = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/calendar/fetch?tokens=${encodeURIComponent(JSON.stringify(tokens))}&calendarId=${encodeURIComponent(calendarId)}`);
    ...
    const handleSyncTo = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/calendar/sync', {
                ...
                body: JSON.stringify({ reservations, tokens, calendarId, targetEmail })
            });
            const result = await response.json();
            if (result.success) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-[50vh] gap-8">
            ...
            <div className="bg-gray-900 border border-blue-900 p-6 rounded text-center w-full max-w-md">
                ...
                <p className="text-gray-400 text-sm mb-1 uppercase tracking-wider">Configuración de Sincronización</p>
                <div className="space-y-3 mt-4">
                    <div className="text-left">
                        <label className="text-[10px] text-gray-500 uppercase font-bold mb-1 block">Email de Destino</label>
                        <input 
                    ...
                    <div className="text-left">
                        <label className="text-[10px] text-gray-500 uppercase font-bold mb-1 block">ID de Calendario</label>
                        <input 
                            className="w-full bg-black border border-gray-800 p-2 text-white text-sm rounded focus:border-[#D2B48C] outline-none"
                            value={calendarId}
                ...
                <div className="mt-4 text-xs text-gray-500 bg-black/30 p-2 rounded">
